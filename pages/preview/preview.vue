					...
					<scroll-view scroll-y>
						<view class="content">
							...
							<!-- <view class="row">
								...
								<text class="value class">明星</text>
							</view> -->
							<view class="row">
								<view class="label">
									发布者:
							...
							<view class="copyright">声明：本图片来用户投稿，非商业使用，用于免费学习交流，如侵犯了您的权益，您可以拷贝壁纸ID举报至平台，邮箱513894357@qq.com,管理将删除侵权壁纸维护您的权益。
							</view>
...
import { apiGetSetupScore, apiWriteDownload, apiShareDetail } from "@/api/api.js";
...
onLoad(async(e) => {
	...
	if(e.type == 'share'){
		let res = await apiShareDetail({id: currentId.value})
		...
		uni.getImageInfo({
			...
			success:(res) => {
				uni.saveImageToPhotosAlbum({
					...
					fail:err => {
						if(err.errMsg == 'saveImageToPhotosAlbum:fail canel'){
							uni.showToast({
								title:"保存失败, 请重新点击下载",
								icon: "none"
							})
...
onShareTimeline ((e) => {
	return {
		title: "虾米壁纸",
		imageUr: "id="+currentId.value+"&type=share"
	}
})
</script>
